...
for idx=1:size(whenInCorrect,1)
  ...
  whenInCorrectSort(idx,3)= whenInCorrect(whenInCorrectValsortPos(idx),3);
end
...
valTestedAnsweredDiffInCorrect=abs(whenInCorrectSort(:,1)-whenInCorrectSort(:,3));
nbWithinOne=numel(find(valTestedAnsweredDiffInCorrect==1));
nbWithinOnewhenIncorrectPercnt=nbWithinOne/nbInCorrectAnswers*100;
nbWithinTwo=numel(find(valTestedAnsweredDiffInCorrect==2));
nbWithinTwowhenIncorrectPercnt=nbWithinTwo/nbInCorrectAnswers*100;

ResPercent=[answeredPercent, correctPercent, nbTrials, nbWithinOnewhenIncorrectPercnt, nbWithinTwowhenIncorrectPercnt];
ResAns=[minValTestedAnswered,maxValTestedAnswered,minReactTimeAnswered,maxReactTimeAnswered,NaN,];
ResNotAns=[minValTestedNotAnswered,maxValTestedNotAnswered, NaN, NaN, NaN,];
ResCorr=[minValTestedCorrect,maxValTestedCorrect,minReactTimeCorrect,maxReactTimeCorrect,NaN,];
ResInCorr=[minValTestedInCorrect,maxValTestedInCorrect,minReactTimeInCorrect,maxReactTimeInCorrect,NaN,];
...
headerRow_TotalRes={'AnsPerc-CorrPer-totalTrials--nbWithinOnewhenIncorrectPercnt--nbWithinTwowhenIncorrectPercnt';....
    'Answered'; 'NotAnswered'; 'Correct'; 'Incorrect'};
